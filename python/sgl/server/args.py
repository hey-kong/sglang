...
import torch
from sgl.distributed import DistributedInfo
from sgl.scheduler import SchedulerConfig
from sgl.utils import init_logger
...
class ServerArgs(SchedulerConfig):
    ...
    def zmq_frontend_addr(self) -> str:
        return "ipc:///tmp/sgl_3" + self._unique_suffix

    @property
    def zmq_tokenizer_addr(self) -> str:
        if self.share_tokenizer:
            return self.zmq_detokenizer_addr
        result = "ipc:///tmp/sgl_4" + self._unique_suffix
...
def parse_args(args: List[str], run_shell: bool = False) -> Tuple[ServerArgs, bool]:
    ...
    """
    from sgl.attention import validate_attn_backend
    from sgl.kvcache import SUPPORTED_CACHE_MANAGER
    from sgl.moe import SUPPORTED_MOE_BACKENDS

    parser = argparse.ArgumentParser(description="SGLang Server Arguments")
    ...
    if (dtype_str := kwargs["dtype"]) == "auto":
        from sgl.utils import cached_load_hf_config
